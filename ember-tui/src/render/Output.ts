...
export default class Output {
	...
	/**
	 * Clear a full row in the buffer directly, resetting every cell to a plain space.
	 * More efficient than writing a string of spaces via write() because it skips
	 * ANSI tokenization and transformer application.
	 */
	clearRow(y: number): void {
		...
		for (let x = 0; x < this.width; x++) {
			row[x] = { type: 'char', value: ' ', fullWidth: false, styles: [] };
		}
		// Truncate any extra cells beyond the current width
